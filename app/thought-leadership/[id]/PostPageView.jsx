...
export default function PostPageView({ post }) {
  useEffect(() => {
  ...
  const shareX = () =>
    window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(getUrl())}`, '_blank', 'noopener');
  ...
  const readingTime = Math.max(1, Math.ceil((post.contentMarkdown || '').split(/\s+/).filter(Boolean).length / 200));
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <main className="max-w-3xl mx-auto px-6 py-10">
        ...
        {post.geoKeywords?.length > 0 && (
          <div className="flex flex-wrap gap-1.5 mb-5 print-hide">
            {post.geoKeywords.map(k => (
              <span key={k} className="text-xs px-2.5 py-0.5 rounded-full bg-blue-950 text-blue-300 border border-blue-800/70">{k}</span>
            ))}
          </div>
        )}

        <h1 className="sa-title text-3xl md:text-4xl font-black text-white leading-tight mb-4">{post.title}</h1>

        {post.dek && (
          <p className="sa-dek text-slate-400 text-lg leading-relaxed mb-5">{post.dek}</p>
        )}

        <div className="flex items-center gap-2 text-xs text-slate-600 mb-8 sa-meta">
          {post.publishedAt && (
        ...
        <div className="border-t border-slate-800 pt-8 pb-12 print-article">
          <StandaloneMarkdown md={post.contentMarkdown || post.excerpt || ''} />
        </div>
        ...
        <div className="print-hide border-t border-slate-800 pt-10 pb-16 flex flex-col items-center gap-5">
          <p className="text-slate-500 text-sm font-medium">Share this intelligence brief</p>
