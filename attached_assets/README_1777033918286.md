...
| `FMP_API_KEY` | No* | — | [Financial Modeling Prep](https://site.financialmodelingprep.com/developer/docs) — search tickers, prices, and fundamentals for the **Investment Thesis** and Economic/Social data blocks (*required to resolve a company to a listed symbol) |
| `FRED_API_KEY` | No | — | [FRED](https://fred.stlouisfed.org/docs/api/api_key.html) — macro series (Fed funds, 10Y, curve, unemployment) in the data snapshot |
| `ADANOS_API_KEY` | No | — | [Adanos](https://adanos.org/register) — cross-platform market sentiment for Social + thesis |
| `TAVILY_API_KEY` | No | — | [Tavily](https://tavily.com) — fresh news sources for dimension agents (see `app/api/research`) |
| `CEREBRAS_API_KEY` | No | — | Optional Cerebras models in the catalog |

On Vercel, the app cannot run the Python [OpenBB](https://github.com/OpenBB-finance/OpenBB) package. Market and macro data use FMP + FRED instead, in the same spirit as an OpenBB-style workflow.
...
│       ├── research/route.js   # POST /api/research  — Tavily (optional)
│       ├── resolve-equity/route.js  # POST — FMP company → ticker
│       ├── market-data/route.js     # POST — FMP + FRED snapshot
│       ├── adanos/route.js     # GET/POST — Adanos sentiment proxy
│       ├── pull/route.js       # POST /api/pull    — returns 410 (not applicable)
│       └── analyze/route.js    # POST /api/analyze — proxies to Groq with retry
├── lib/
│   └── marketData/             # FMP + FRED helpers, technicals (RSI, SMAs)
