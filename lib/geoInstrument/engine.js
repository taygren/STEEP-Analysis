...
export function buildConvergencePrompt(instrument, sender, target, context, agent5a, agent5b, agent5c) {
  const ctx = context?.trim() ? `\nContext: ${context}` : '';
  const s5a = `Severity ${agent5a?.severity_tier} (${Number(agent5a?.severity_score ?? 0).toFixed(1)}/10), dominant: ${agent5a?.dominant_attribute}, lowest: ${agent5a?.lowest_attribute}`;
...
Active flag rules — apply all that are true:
HIGH_SEVERITY: severity_score ≥ 7
RETALIATION_RISK: retaliation_capacity = HIGH OR escalation_probability ≥ 50
CIRCUMVENTION_LIKELY: attribute_scores.circumvention ≤ 4
WAR_ECONOMY_TRIGGER: escalation_probability ≥ 70 AND leverage_holder = "sender" AND retaliation_capacity = "HIGH"
...
export const GEO_5D_TOKENS          = 600;
