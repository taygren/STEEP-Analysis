...
// ── Public API ────────────────────────────────────────────────────
function getKv() {
  if (!globalThis.__steep_kv_instance) {
    globalThis.__steep_kv_instance = fileKv;
  }
  return globalThis.__steep_kv_instance;
}

export async function kvGet(key)                 { return getKv().get(key); }
export async function kvSet(key, value)          { return getKv().set(key, value); }
export async function kvDel(key)                 { return getKv().del(key); }
export async function kvKeys(pattern)            { return getKv().keys(pattern); }
export async function kvZAdd(key, score, member) { return getKv().zadd(key, score, member); }
export async function kvZRange(key, s, e, opts)  { return getKv().zrange(key, s, e, opts); }
export async function kvZRem(key, member)        { return getKv().zrem(key, member); }
