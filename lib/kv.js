...
// ── Public API ────────────────────────────────────────────────────
// Store the resolved kv instance in globalThis so Next.js hot-module
// reloads (which re-execute module scope and reset let _kv = null) do
// not create a new instance and accidentally bypass the loaded store.
async function getKv() {
  if (globalThis.__steep_kv_instance) return globalThis.__steep_kv_instance;

  if (process.env.KV_REST_API_URL && process.env.KV_REST_API_TOKEN) {
    try {
      const { kv } = await import('@vercel/kv');
      globalThis.__steep_kv_instance = kv;
